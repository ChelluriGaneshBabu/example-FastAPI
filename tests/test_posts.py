...
def test_get_one_post_not_exist(authorized_client,test_post):
    res = authorized_client.get(f"/posts/456546")
...
def test_create_post(authorized_client,test_user,title,content,published):
    ...
    created_post = schema.Post_response(**res.json())
...
def test_create_post_default_published_true(authorized_client,test_user):
    res = authorized_client.post("/posts/",json={"title":"ganesh","content":"tall"})
    created_post = schema.Post_response(**res.json())
    assert res.status_code == 201
    assert created_post.owner_id == test_user['id']
    assert created_post.title == "ganesh"
    assert created_post.content == "tall"
    assert created_post.published == True

def test_unauthorized_user_create_post(client,test_user,test_post):
    res = client.post("/posts/",json={"title":"ganesh","content":"tall","published":True})
    assert res.status_code == 401
   
def test_unauhtorized_user_delete_post(client,test_user,test_post):
...
def test_delete_post_not_exist(authorized_client,test_user,test_post):
    res = authorized_client.delete(f"/posts/556")
    assert res.status_code == 404
...
def test_update_post(authorized_client,test_user,test_post):
    data = {"title":"updated post","content":"updated content","id":test_post[0].id}
    res = authorized_client.put(f"/posts/{test_post[0].id}",json =data)
    updated_post = schema.Post_response(**res.json())
    assert res.status_code == 200
    assert updated_post.title == data["title"]
    assert updated_post.content == data["content"]

def test_update_other_user_post(authorized_client,test_post,test_user,test_user2):
    data = {"title":"updated post","content":"updated content","id":test_post[3].id}
...
def test_update_post_not_exist(authorized_client,test_post,test_user):
    data = {"title":"updated post","content":"updated content","id":test_post[3].id}
    res = authorized_client.put(f"/posts/543543",json=data)
    assert res.status_code == 404
